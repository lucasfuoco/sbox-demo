...
public static partial class GameUtils
{
	...
	/// </summary>
	public static IEnumerable<ClientComponent> AllPlayers => Game.ActiveScene.GetAllComponents<ClientComponent>();

	/// <summary>
	/// Get all players on a team.
	/// </summary>
	public static IEnumerable<ClientComponent> GetPlayers( Team team ) => AllPlayers.Where( x => x.Team == team );

	/// <summary>
	/// Every <see cref="PlayerPawn"/> currently in the world.
	/// </summary>
	public static IEnumerable<PlayerPawnComponent> PlayerPawns => AllPlayers.Select( x => x.PlayerPawn ).Where( x => x.IsValid() );

	/// <summary>
	/// Every <see cref="PlayerPawn"/> currently in the world, on the given team.
	/// </summary>
	public static IEnumerable<PlayerPawnComponent> GetPlayerPawns( Team team ) => PlayerPawns.Where( x => x.Team == team );
	...
	public static IEnumerable<SpawnPointInfo> GetSpawnPoints( Team team, params string[] tags ) => Game.ActiveScene
		.GetAllComponents<TeamSpawnPointComponent>()
		.Where( x => x.Team == team )
	...
	/// </summary>
	public static PlayerPawnComponent GetPlayerFromComponent( Component component )
	{
		if ( component is PlayerPawnComponent player ) return player;
		if ( !component.IsValid() ) return null;
		return !component.GameObject.IsValid() ? null : component.GameObject.Root.GetComponentInChildren<PlayerPawnComponent>();
	}

	/// <summary>
	/// Get a player from a component that belongs to a player or their descendants.
	/// </summary>
	public static PawnComponent GetPawn( Component component )
	{
		if ( component is PawnComponent pawn ) return pawn;
		if ( !component.IsValid() ) return null;
		return !component.GameObject.IsValid() ? null : component.GameObject.Root.GetComponentInChildren<PawnComponent>();
	}

	public static EquipmentComponent FindEquipment( Component inflictor )
	{
		if ( inflictor is EquipmentComponent equipment )
		{
			return equipment;
		}
