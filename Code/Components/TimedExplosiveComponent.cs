...
using DamageInfo = Sandbox.DamageInfo;

public sealed class TimedExplosiveComponent : Component, IUseValid, IMinimapIconValid, IDescription
{
	...
	/// </summary>
	private BombSiteComponent BombSite { get; set; }

	[RequireComponent] public SpottableComponent Spottable { get; private set; }

	public PlayerPawnComponent DefusingPlayer { get; private set; }
	...
	protected override void OnStart()
	{
		...
		BombSite = ZoneComponent.GetAt( WorldPosition )
			.Select( x => x.GetComponent<BombSiteComponent>() )
	...
	public void StartDefusing( PlayerPawnComponent player )
	{
		if ( player is null )
		{
		...
		{
			RadioSoundsGameResource.Play( player.Team, "Hidden", "Defusing the bomb" );
