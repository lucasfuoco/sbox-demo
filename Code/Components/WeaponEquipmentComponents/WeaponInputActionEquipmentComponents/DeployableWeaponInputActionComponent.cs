...
public partial class DeployableWeaponInputActionComponent : WeaponInputActionEquipmentComponent
{
	...
	protected override void OnInputDown()
	{
		...
		dropped.NetworkSpawn( player.Network.Owner );

		// If it's a pawn, possess it
		// This'll probably get removed
		if ( dropped.GetComponent<PawnComponent>() is PawnComponent pawn && pawn.IsValid() )
		{
			pawn.Client = client;
			pawn.Possess();

			if ( Networking.IsHost )
				DeployedPawnHandlerComponent.Create( pawn, client );
		}
