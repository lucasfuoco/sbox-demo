...
using Sandbox.Components.GlobalComponents;

namespace Sandbox.Components.PawnComponents;

public partial class PlayerPawnComponent : IGameEventHandler<WeaponShotEvent>
{
	/// <summary>
	...
	public PlayerGlobalsGlobalComponent Global => GetGlobal<PlayerGlobalsGlobalComponent>();
	...
	/// What's our holdtype?
	/// </summary>
	[Sync] AnimationHelperComponent.HoldTypes CurrentHoldType { get; set; } = AnimationHelperComponent.HoldTypes.None;
	...
	private void OnUpdateMovement()
	{
		...
		CurrentHoldType = CurrentEquipment.IsValid() ? CurrentEquipment.GetHoldType() : AnimationHelperComponent.HoldTypes.None;
		...
		if ( Body.IsValid() )
		{
			...
			foreach ( var helper in Body.AnimationHelpers )
			{
				...
				helper.MoveStyle = AnimationHelperComponent.MoveStyles.Run;
				helper.DuckLevel = (MathF.Abs( _smoothEyeHeight ) / 32.0f);
				helper.HoldType = CurrentHoldType;
				helper.Handedness = CurrentEquipment.IsValid() ? CurrentEquipment.Handedness : AnimationHelperComponent.Hand.Both;
			}
		}
