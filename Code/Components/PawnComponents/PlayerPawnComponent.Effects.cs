...
public partial class PlayerPawnComponent
{
	...
	private bool IsOutlineVisible()
	{
		...
		if ( SpectateSystemSingletonComponent.Instance.IsValid() && SpectateSystemSingletonComponent.Instance.IsSpectating )
			return true;

		if ( HealthComponent.IsGodMode )
			return true;

		var cl = ClientComponent.Local;
		...
		var viewer = ClientComponent.Viewer;
		if ( viewer.IsValid() )
	...
	private void UpdateOutline()
	{
		...
		if ( ClientComponent.Local.GetLastKiller() == this && SpectateSystemSingletonComponent.Instance.IsValid() && SpectateSystemSingletonComponent.Instance.IsSpectating )
			Outline.ObscuredColor = Color.Red;
		else if ( SpectateSystemSingletonComponent.Instance.IsSpectating )
			Outline.ObscuredColor = Team.GetColor();
		else
			Outline.ObscuredColor = ClientComponent.Viewer.Team == Team
