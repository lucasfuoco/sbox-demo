...
public partial class PlayerPawnComponent : IGameEventHandler<DamageGivenEvent>, IGameEventHandler<DamageTakenEvent>
{
	/// <summary>
	...
	void IGameEventHandler<DamageGivenEvent>.OnGameEvent( DamageGivenEvent eventArgs )
	{
		...
		if ( IsViewer )
		{
			CrosshairComponent.Instance?.Trigger( eventArgs.DamageInfo );
		}
	}
	...
	void IGameEventHandler<DamageTakenEvent>.OnGameEvent( DamageTakenEvent eventArgs )
	{
		...
		if ( IsViewer )
		{
			DamageIndicatorPanel.Current?.OnHit( position );
		}
