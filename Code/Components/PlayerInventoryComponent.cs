...
{
	[RequireComponent] public PlayerPawnComponent Player { get; set; }

	/// <summary>
	/// What equipment do we have right now?
	/// </summary>
	public IEnumerable<EquipmentComponent> Equipment => Player.GetComponentsInChildren<EquipmentComponent>();

	/// <summary>
	/// Does this player have a bomb on them?
	/// </summary>
	public bool HasBomb => Equipment.Any( x => x.GetComponentInChildren<C4DefuseWeaponInputActionEquipmentComponent>().IsValid() );
	...
	/// </summary>
	private EquipmentComponent Current => Player.CurrentEquipment;
	...
	public void RefillAmmo()
	{
		foreach ( var wpn in Equipment )
		{
			if ( wpn.GetComponentInChildren<WeaponAmmoComponent>() is { } ammo )
			{
	...
	public void Drop( EquipmentComponent weapon, bool forceRemove = false )
	{
	...
	[Rpc.Broadcast]
	private void DropHost( EquipmentComponent weapon, bool forceRemove )
	{
		if ( !Networking.IsHost )
			return;
		...
		var canDrop = GameModeSingletonComponent.Instance.Get<EquipmentDropperComponent>() is not { } dropper || dropper.CanDrop( Player, weapon );
		...
		if ( canDrop )
		{
			...
			var droppedWeapon = DroppedEquipmentComponent.Create( weapon.Resource, position, rotation, weapon );
	...
	public void Switch( EquipmentComponent equipment )
	{
		Assert.True( !IsProxy || Networking.IsHost );
	...
	/// </summary>
	public void RemoveWeapon( EquipmentComponent equipment )
	{
	...
	public EquipmentComponent Give( EquipmentResource resource, bool makeActive = true )
	{
		Assert.True( Networking.IsHost );
