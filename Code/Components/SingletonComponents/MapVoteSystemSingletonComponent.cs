...
[Hide]
public partial class MapVoteSystemSingletonComponent : SingletonComponent<MapVoteSystemSingletonComponent>
{
	/// <summary>
	...
	/// </summary>
	[RequireComponent] MapVoteOverlayPanel MapVoteOverlay { get; set; }
	...
	protected override void OnUpdate()
	{
		...
		if ( WinningOption.HasValue && TimeUntilTransfer )
		{
			...
			GameModeSingletonComponent.SetCurrent( option.Mode );
	...
	public static IEnumerable<Option> GenerateOptions( GameModeSingletonComponent currentMode = null, SceneFile currentMap = null, int count = 5 )
	{
		var maps = ResourceLibrary.GetAll<SceneFile>().Where( x => x.GetMetadata( "IsVisibleInMenu", null ).ToBool() is true ).ToList();
		var options = new List<Option>();

		var inList = ( GameModeSingletonComponent gameMode ) =>
		{
		...
		while ( options.Count < count )
		{
			...
			var modes = GameModeSingletonComponent.GetAll( map );
	...
	/// <returns></returns>
	public static MapVoteSystemSingletonComponent Create( GameModeSingletonComponent currentMode = null, SceneFile currentMap = null, int count = 5 )
	{
		// Host-only
		...
		var system = go.Components.Create<MapVoteSystemSingletonComponent>();
	...
	public record struct Option
	{
		...
		/// </summary>
		public GameModeSingletonComponent Mode { get; set; }
	...
	public struct NetworkedOption
	{
		...
		public Option AsNormal()
		{
			...
			var mode = GameModeSingletonComponent.GetAll( mapDef )
