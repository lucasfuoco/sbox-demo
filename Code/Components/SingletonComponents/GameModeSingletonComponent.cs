...
/// <summary>
/// Posted on the scene when the active game mode has started. Implement on a <see cref="Component"/> to react.
/// </summary>
public interface IGamemodeInitializedEvent : ISceneEvent<IGamemodeInitializedEvent>
{
	void OnGamemodeInitialized( string title );
}
...
/// </summary>
public sealed partial class GameModeSingletonComponent : SingletonComponent<GameModeSingletonComponent>, Component.INetworkListener
{
	/// <summary>
	...
	public static void SetCurrent( GameModeSingletonComponent gameMode )
	{
		if ( gameMode is null )
		{
			ActivePath = null;
	...
	{
		Scene.RunEvent<IGamemodeInitializedEvent>( e => e.OnGamemodeInitialized( Title ), default );
		...
		where T : class
	{
		...
		{
			throw new Exception( $"Expected a {typeof( T ).Name} to be active in the {nameof( GameModeSingletonComponent )}!" );
		}
