...
using static Sandbox.Components.ClientComponent;
...
{
	public static CrosshairComponent Instance { get; set; }
	...
	protected override void OnUpdate()
	{
		...
		if ( !ClientComponent.Viewer.Pawn.Camera.IsValid() )
			return;

		if ( !MainHUDPanel.IsHudEnabled )
			return;

		float alphaTarget = 1f;
		float linesTarget = 1f;

		var pawn = ClientComponent.Viewer.Pawn;
		var hud = ClientComponent.Viewer.Pawn.Camera.Hud;
		...
		var player = pawn as PlayerPawnComponent;
		if ( player.IsValid() && player.CameraController.IsValid() )
		{
			...
			if ( equipment.IsValid() )
			{
				...
				var recoil = equipment.GetComponentInChildren<ShootRecoilEquipmentComponent>( true );
				if ( recoil.IsValid() )
				{
	...
	{
		if ( damageInfo.Victim is PlayerPawnComponent victim && victim.IsValid() )
		{
