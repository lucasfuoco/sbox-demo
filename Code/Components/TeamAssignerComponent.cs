...
/// </summary>
public sealed class TeamAssignerComponent : Component,
	...
	IGameEventHandler<PlayerJoinedEvent>
{
	...
	private float GetScore( ClientComponent player )
	{
		return AutoBalanceMode switch
		{
			AutoBalanceMode.Random => Random.Shared.NextSingle(),
			AutoBalanceMode.LowestScorers => player.GetComponent<PlayerScoreComponent>() is { } playerScore ? playerScore.Score : 0,
			AutoBalanceMode.LowestScorersLastRound => player.GetComponent<PlayerScoreComponent>() is { } playerScore ? playerScore.ScoreHistory.LastOrDefault() : 0,
			_ => 0
		};
	}

	private void AssignTeam( ClientComponent player, bool dispatch )
	{
