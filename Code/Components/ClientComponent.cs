...
public partial class ClientComponent : Component, ITeamValid
{
	/// <summary>
	/// The player we're currently in the view of (clientside).
	/// Usually the local player, apart from when spectating etc.
	/// </summary>
	public static ClientComponent Viewer { get; private set; }

	/// <summary>
	/// Our local player on this client.
	/// </summary>
	public static ClientComponent Local { get; private set; }
	...
	private string name => IsBot ? $"BOT {BotManagerSingletonComponent.Instance.GetName( BotId )}" : SteamName ?? "";
	/// <summary>
	/// Name of this player
	/// </summary>
	...
	/// What's our loadout?
	/// </summary>
	[RequireComponent] public PlayerLoadoutComponent Loadout { get; private set; }
	...
	/// </summary>
	[Sync( SyncFlags.FromHost ), ValidOrNull] public PlayerPawnComponent PlayerPawn { get; set; }

	/// <summary>
	/// The pawn this player is currently in possession of (synced - unless the pawn is not networked)
	/// </summary>
	[Sync] public PawnComponent Pawn { get; set; }

	public void HostInit()
	{
		var defaultRespawnState = Scene.GetAllComponents<DefaultRespawnStateComponent>().FirstOrDefault();
	...
	public static void OnPossess( PawnComponent pawn )
	{
		if ( !pawn.IsValid() )
		{
