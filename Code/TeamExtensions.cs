...
public static class TeamExtensions
{
	...
	{
		if ( gameObject.Root.GetComponent<PawnComponent>() is { IsValid: true } pawn )
		{
			return pawn.Team;
		}

		if ( gameObject.Root.GetComponent<ClientComponent>() is { IsValid: true } state )
		{
			return state.Team;
		}
	...
	/// </summary>
	public static bool IsFriendly( this PlayerPawnComponent self, PlayerPawnComponent other )
	{
		if ( !self.IsValid() || !other.IsValid() ) return false;
	...
	/// </summary>
	public static bool IsFriendly( this ClientComponent self, ClientComponent other )
	{
		if ( !self.IsValid() || !other.IsValid() ) return false;
		return IsFriendly( self.Team, other.Team );
	}
