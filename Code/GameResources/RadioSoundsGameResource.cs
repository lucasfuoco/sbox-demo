...
public class RadioSoundsGameResource : GameResource
{
	public static HashSet<RadioSoundsGameResource> All { get; set; } = new();
	...
	public static void Play( Team team, string category, string sound )
	{
		...
		using ( Rpc.FilterInclude( GameUtils.GetPlayers( team ).Select( x => x.Connection ) ) )
		{
			ChatPanel.Instance?.AddText( sound, ChatPanel.ChatModes.Team, "radio" );
	...
	public static void CmdPlay( string category, string sound )
	{
		...
		var team = ClientComponent.Local.Team;
	...
	private static void RpcPlay( Team team, string category, string sound )
	{
		...
		var localPlayerTeam = ClientComponent.Local.Team;
