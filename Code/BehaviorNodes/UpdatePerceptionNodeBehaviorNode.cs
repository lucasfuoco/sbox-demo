...
public class UpdatePerceptionNodeBehaviorNode : BaseBehaviorNode
{
	...
	public UpdatePerceptionNodeBehaviorNode( float range = 2048f )
	{
		_range = range;
	}
	...
	protected override NodeResult OnEvaluate( BotContext context )
	{
		...
		var enemies = new List<PawnComponent>();
		var teammates = new List<PawnComponent>();
		var items = new List<Component>();

		// Scan all pawns
		foreach ( var otherPawn in pawn.Scene.GetAll<PlayerPawnComponent>() )
		{
			if ( otherPawn == pawn )
				continue;
		...
		foreach ( var pickup in pawn.Scene.GetAll<DroppedEquipmentComponent>() )
		{
	...
	private bool IsInLineOfSight( PawnComponent from, Component obj )
	{
		var targetPos = obj is PawnComponent p ? p.EyePosition : obj.WorldPosition;
