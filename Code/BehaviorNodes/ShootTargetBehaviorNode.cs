...
/// </summary>
public class ShootTargetBehaviorNode : BaseBehaviorNode
{
	...
	public ShootTargetBehaviorNode( RangedFloat? burstRange = null, float burstBreakTime = 0.5f )
	{
	...
	protected override NodeResult OnEvaluate( BotContext context )
	{
		...
		var target = context.GetData<PlayerPawnComponent>( CURRENT_TARGET_KEY );
		...
		if ( !weapon.IsValid() || weapon.GetComponentInChildren<ShootableWeaponInputActionEquipmentComponent>() is not { } shootable )
			return NodeResult.Failure;
