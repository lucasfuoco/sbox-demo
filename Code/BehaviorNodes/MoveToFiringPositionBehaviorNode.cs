...
/// </summary>
public class MoveToFiringPositionBehaviorNode : BaseBehaviorNode
{
	...
	protected override NodeResult OnEvaluate( BotContext context )
	{
		...
		var target = context.GetData<PawnComponent>( CURRENT_TARGET_KEY );
	...
	private bool IsTargetVisibleFromPerception( BotContext context, PawnComponent target )
	{
		if ( !context.HasData( "visible_enemies" ) )
			return false;

		var visibleEnemies = context.GetData<List<PawnComponent>>( "visible_enemies" );
