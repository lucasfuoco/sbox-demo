...
public abstract class ScreenShake
{
	...
	public class Fov : ScreenShake
	{
		...
		public override bool Update( CameraComponent camera )
		{
			...
			if ( !ClientComponent.Viewer.PlayerPawn.IsValid() )
				return false;

			if ( !ClientComponent.Viewer.PlayerPawn.CameraController.IsValid() )
				return false;

			var c = Curve.Evaluate( Progress );

			var cc = ClientComponent.Viewer.PlayerPawn?.CameraController;
